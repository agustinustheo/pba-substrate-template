...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn increment(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
            ...
            // Emit an event.
            Self::deposit_event(Event::CounterIncremented(new));
        ...
        pub fn decrement(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
            ...
            let current = <Counter<T>>::get().unwrap_or(0);
            let new = current.checked_sub(1).ok_or(Error::<T>::Underflow)?;
            <Counter<T>>::put(new);

            // Emit an event.
            Self::deposit_event(Event::CounterDecremented(new));
